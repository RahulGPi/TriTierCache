#!/usr/bin/env python3
"""
benchmarks/run_llama_graduated_niah.py
Graduated NIAH sweep on meta-llama/Llama-3.2-1B using its ACTUAL RoPE base context (8192 tokens):
- max_position_embeddings: 131072
- rope_scaling: llama3 (factor 32.0, low_freq 1.0, high_freq 4.0, orig_max 8192)
- Evaluates lengths across base context boundary:
    1.0x (8192), 1.1x (9011), 1.2x (9830), 1.25x (10240),
    1.3x (10650), 1.4x (11469), 1.5x (12288), 1.75x (14336), 2.0x (16384)
- Depths: 0.20, 0.70, 0.95
- RoPE modes: 'a' (native absolute Llama 3 RoPE) vs 'b' (window-clamped)
- H_ratios: 0.05, 0.15
- Records exact storage tier, mean K cosine, mean V cosine, storage_intact, and retrieval success.
Emits:
  benchmarks/results/llama_base8192_graduated_niah.csv
"""
...
from benchmarks.analyze_niah_failure import construct_niah_prompt
...
def evaluate_llama_niah_row(model, tok, total_len: int, depth: float, rope_mode: str, h_ratio: float = 0.05, needle_key: str = "94821"):
    ...
    # Arm needle capture hook on all layers
    ...
    with torch.no_grad():
        ...
        for step in range(4):
            ...
            if rope_mode == "a":
                # Native Llama 3 RoPE scaling
                pos_tensor = torch.tensor([[dec_pos]], dtype=torch.int64)
            else:
                # Mode 'b' (clamped relative window pos)
                rel_pos = min(dec_pos, patch_mod.R_SIZE + 4 + step)
    ...
    # Reconstruct needle K/V from cache
    k_cos_sims = []
    v_cos_sims = []
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        ...
        # Check RW (Recent Window) using the invariant ring-buffer indexing formula
...
def main():
    ...
    cfg = model.config
    print(f"Llama-3.2-1B RoPE Config:")
    print(f"  max_position_embeddings: {cfg.max_position_embeddings}")
    print(f"  rope_scaling: {cfg.rope_scaling}")
    head_dim = cfg.hidden_size // cfg.num_attention_heads
    print(f"  num_attention_heads: {cfg.num_attention_heads}, num_key_value_heads: {cfg.num_key_value_heads}, head_dim: {head_dim}")
    ...
    output_csv = "benchmarks/results/llama_base8192_graduated_niah.csv"
    ...
    results = []
    print("=" * 145)
    print(f"{'Ratio':<7} | {'Context':<8} | {'Depth':<6} | {'RoPE':<5} | {'H_ratio':<7} | {'Tier':<6} | {'K Cos':<12} | {'V Cos':<12} | {'Intact':<7} | {'Success':<8} | {'Elapsed':<7}")
    print("-" * 145)
    ...
    for ratio, length in lengths:
        for h_ratio in h_ratios:
            for mode in rope_modes:
                for depth in depths:
                    res = evaluate_llama_niah_row(model, tok, length, depth, mode, h_ratio)
                    row = {
                        "model": model_id,
                        "length_ratio": ratio,
                    ...
                    print(f"{ratio:<7.2f} | {res['context_length']:<8d} | {depth:<6.2f} | {mode:<5} | {h_ratio:<7.2f} | {res['dominant_tier']:<6} | {res['mean_k_cosine_sim']:<12.8f} | {res['mean_v_cosine_sim']:<12.8f} | {intact_str:<7} | {succ_str:<8} | {res['elapsed']:<7.2f}s", flush=True)

    print("=" * 145)
    print(f"Llama-3.2-1B base 8192 graduated sweep complete. Saved {len(results)} rows to {output_csv}")
