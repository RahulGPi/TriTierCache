#!/usr/bin/env python3
"""
benchmarks/run_llama_nonrepeating_graduated_niah.py
Graduated NIAH sweep on meta-llama/Llama-3.2-1B using its ACTUAL RoPE base context (8192 tokens)
over the DIVERSE, NON-REPEATING corpus (corpus.py):
...
Emits:
  benchmarks/results/llama_nonrepeating_base8192_graduated_niah.csv
"""
...
def evaluate_llama_niah_row(model, tok, total_len: int, depth: float, rope_mode: str, h_ratio: float = 0.05, needle_key: str = "94821", is_vanilla: bool = False):
    full_prompt, needle_start, needle_end, key_pos = construct_nonrepeating_niah_prompt(tok, total_len, depth, needle_key)
    ...
    if is_vanilla:
        ...
        t0 = time.time()
        with torch.no_grad():
            base_out = model(full_prompt, use_cache=True)
            past_kv = base_out.past_key_values
            curr_id = base_out.logits[:, -1, :].argmax(dim=-1, keepdim=True)
    ...
    with torch.no_grad():
        ...
        for step in range(8):
            ...
            if rope_mode == "a":
                ...
                rel_pos = min(dec_pos, patch_mod.R_SIZE + 8 + step)
    ...
    for layer_idx, layer in enumerate(model.model.layers):
        ...
        # Check RW (Recent Window) using invariant ring-buffer indexing
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Llama-3.2-1B Base 8192 Graduated NIAH Sweep")
    parser.add_argument("--include-vanilla", action="store_true", help="Include Vanilla FP32 baseline")
    parser.add_argument("--output", type=str, default="benchmarks/results/llama_nonrepeating_base8192_graduated_niah.csv")
    ...
    if args.include_vanilla:
        ...
        for ratio, length in lengths:
            for depth in depths:
                res = evaluate_llama_niah_row(model, tok, length, depth, rope_mode="vanilla", is_vanilla=True)
                row = {
                    "model": model_id,
                    "length_ratio": ratio,
    ...
    print("=" * 145)
    print(f"Non-repeating Llama graduated sweep complete. Saved {len(results)} rows to {output_csv}")
